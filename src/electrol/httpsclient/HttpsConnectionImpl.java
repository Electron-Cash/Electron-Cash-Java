...
public class HttpsConnectionImpl implements HttpsConnection {
	...
	protected void connect() throws IOException {
		ensureSetupState();
		connection = (SocketConnection) Connector.open("socket://"+ this.getHost() + ":" + this.getPort(),Connector.READ_WRITE, true);
		connection.setSocketOption(SocketConnection.KEEPALIVE, 1);
		tlsClientProtocol = new TlsClientProtocol(connection.openInputStream(),connection.openOutputStream());
		DefaultTlsClient tlsClient = new TlsClientWithCertificateValidator(new CertificateValidator(this));
	...
	public void connectSocket() {
		...
		switchState(CONN_STATE_CONNECTED);
			...
			System.out.println("can not connect with server!!! TCP error with ");
			e.printStackTrace();
		}
	}
