...
public class CertificateValidatorUtils {
	...
	public static List extractSubjectAlternativeNameList(
		...
		Extensions extension = cert.getTBSCertificate()
				.getExtensions();
		if(extension == null) {
			return dnsNames;
		}
		Extension subjectAlternativeName = extension.getExtension(Extension.subjectAlternativeName);
	...
	private static Boolean isTrustedCertificate(Certificate cert,
		...
		Certificate repositoryCertificate = cert;//getCertificateFromJarOrRecordStore(
	...
	private static Boolean isHostAunthenticationCertificate(Certificate cert)
		...
		Extensions extentions= cert.getTBSCertificate().getExtensions();
		if(extentions == null) {
			return Boolean.FALSE;
		}
		Extension extKeyUsageExtension = extentions.getExtension(Extension.extendedKeyUsage);
	 ...
	 * @throws IOException
	 */
	public static org.bouncycastle.asn1.x509.Certificate getCertificateFromJarOrRecordStore1(
		...
		if (certBA == null) {
			...
			certBA = loadJarResource("/" + friendlyCN + ".der");
		}
