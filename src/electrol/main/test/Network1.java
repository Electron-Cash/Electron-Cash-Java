...
import electrol.main.Deserialize;
import electrol.main.MessagesCallbackTuple;
import electrol.main.MethodParamsTuple;
import electrol.main.NetworkUtil;
import electrol.main.RequestQueueItem;
import electrol.main.RequetResponseTuple;
import electrol.main.Storage;
import electrol.util.Server;

public class Network1 extends Thread{
	...
	public Network1(ApplicationContext context, Storage storage){
	...
	private void get_raw_tx(JSONObject response) throws JSONException {
		...
		System.out.println(d.getLockTime());
	...
	private void get_tx(JSONObject response) throws JSONException {
		...
		for(int i=0;i<result.length();i++) {
			...
			if(!verified_tx3.has(hash)) {
				verified_tx3.put(hash, new JSONObject());
			}
			if(!transactions.has(hash)) {
				transactions.put(hash, "");
			}
		}
		storage.put("verified_tx3", verified_tx3);
		storage.put("txi", txi);
		storage.put("txo", txo);
		storage.put("transactions", transactions);
