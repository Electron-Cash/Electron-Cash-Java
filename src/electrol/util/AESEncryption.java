...
public class AESEncryption {
    ...
    public byte[] encrypt(String message) throws InvalidCipherTextException, IOException {
    	return encrypt(message.getBytes());
    }
    
    public byte[] encrypt(byte[] message) throws InvalidCipherTextException, IOException {
    	byte[] out = process(message,true);
        return Base64.encode(Arrays.concatenate("BIE1".getBytes(), out));
    }
 
    
    public byte[] decrypt(String message) throws InvalidCipherTextException, IOException {
    	byte[] out = Base64.decode(message);
        return decryptBytes(out);
    }
    
    public byte[] decrypt(byte[] message) throws IOException, InvalidCipherTextException {
        byte[] out = Base64.decode(message);
        return decryptBytes(out);
    }
    
    private byte[] decryptBytes(byte[] out) throws InvalidCipherTextException {
    	String magic = new String(electrol.java.util.Arrays.slice(out, 0, 4));
        if("BIE1".equals(magic)) {
        	byte[] pre = electrol.java.util.Arrays.slice(out, 4, out.length);
    ...
    public void init(byte[] secret) {
    	...
    	byte[] dk = ((KeyParameter) gen.generateDerivedParameters(64*8)).getKey();
