...
import org.electroncash.util.Arrays;
  ...
  implements CommandListener
{
  ...
  public HistoryCanvas(Electron electron) {
    ...
    try {
      table = new ScrollableTable(get_history(), getWidth() - 20, getHeight() - 20);
  ...
  {
    Storage storage = electron.getStorage();
    ...
    for (int k = 0; k < object.length(); k++) {
      ...
      while (e.hasMoreElements()) {
        ...
        if (keys[k] == tmpTime) {
          hash[k] = key;
          date.setTime(tmpTime * 1000L);
          float tx_delta = get_tx_delta(storage, key, array.optInt(2)) / 200.0F;
          amount += tx_delta;
