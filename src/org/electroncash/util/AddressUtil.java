...
public class AddressUtil
{
  ...
  public static final int OP_SINGLEBYTE_END = 240;
  ...
  private static final BigInteger[] POLYMOD_GENERATORS = { new BigInteger("98f2bc8e61", 16), 
  ...
  public static BitcoinCashAddressDecodedParts decodeCashAddress(String bitcoinCashAddress) {
    ...
    String[] addressParts = StringUtils.split(bitcoinCashAddress, ":");
  ...
  public static String generateCashAddrFromLegacyAddr(String legacyAddr) {
    ...
    return toCashAddress((byte)0, address).substring("bitcoincash:".length());
  }
  ...
  public String[] generateCashAddresses(String[] pubKeyHash) {
    ...
    for (int i = 0; i < pubKeyHash.length; i++) {
      ...
      String cash_address = toCashAddress((byte)0, pubKeyHashBytes);
  ...
  public String generateOneCashAddress(int number, int address_type) {
    ...
    return toCashAddress((byte)0, pubKeyHashBytes);
  }
  ...
  public static boolean isValidCashAddress(String bitcoinCashAddress) {
    try {
      ...
      byte[] checksumData = Arrays.concatenate(Arrays.concatenate(getPrefixBytes(prefix), new byte[1]), 
  ...
  public static String legacyAddressDecode(String v)
  {
    ...
    for (int counter = 0; counter <= vBytes.length - 1; counter++) {
      ...
      charpos = "123456789ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz".indexOf(c);
  ...
  public static String encodeAddress(String addressBytes) {
    try {
      ...
      return Base58.encode(ByteUtilities.toByteArray(encodedBytes));
    }
    catch (Exception e) {}
    return null;
  }
