...
public class Transaction
{
  ...
  public static String serializePreimage(int i, int localBlockHeight, int prevout_n, TxIn[] txins, TxOut[] txouts)
  {
    String hashSequence = "";
    String hashPrevouts = "";
    
    for (int j = 0; j < txins.length; j++) {
      hashSequence = hashSequence + "feffffff";
      hashPrevouts = hashPrevouts + serializeOutpoint(txins[j]);
    }
    ...
    for (int j = 0; j < txouts.length; j++) {
      hashOutputs = hashOutputs + serializeOutput(txouts[j]);
    }
    
    hashOutputs = Bitcoin.Hash(hashOutputs);
    String outpoint = serializeOutpoint(txins[i]);
    long satoshiAmount = txins[i].getAmount();
    ...
    String amount = BlockchainsUtil.long_to_hex(satoshiAmount, 8);
    String nLocktime = BlockchainsUtil.int_to_hex(localBlockHeight, 4);
    
    String preimage = "01000000" + hashPrevouts + hashSequence + outpoint + scriptCode + amount + "feffffff" + 
      hashOutputs + nLocktime + "41000000";
    System.out.println(prevout_n);
    return preimage;
  }
  ...
  public static String serializeInput(TxIn txin)
  {
    ...
    String retval = s + Util.var_int(length) + script + "feffffff";
    return retval;
  }
  ...
  public static String serializeOutput(TxOut txout)
  {
    ...
    long satoshiAmount = txout.getAmount();
    String satoshiAmountHex = BlockchainsUtil.long_to_hex(satoshiAmount, 8);
    String script;
    if (addr.length() > 35) {
      if (!addr.substring(0, 11).equals("bitcoincash")) {
  ...
  public static String serialize(TxIn[] txins, TxOut[] txouts, String nVersion, String nLocktime) {
    ...
    for (int i = 0; i < txins.length; i++) {
      input_string = input_string + serializeInput(txins[i]);
    }
    ...
    for (int i = 0; i < txouts.length; i++) {
      output_string = output_string + serializeOutput(txouts[i]);
    }
  ...
  public TxIn[] Sign(TxIn[] inputs, TxOut[] outputs) throws JSONException {
    ...
    int localBlockHeight = Wallet.getLocalBlockHeight();
  ...
  public String Generate() throws JSONException {
    ...
    return serialize(inputs, outputs, "01000000", nLocktime);
  }
