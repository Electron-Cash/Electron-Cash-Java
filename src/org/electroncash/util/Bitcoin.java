...
public class Bitcoin
{
  ...
  public static BigInteger bigPow(BigInteger base, BigInteger exponent) { BigInteger result = BigInteger.ONE;
  ...
  public static String base_decode_58(String v)
  {
    ...
    for (int counter = vBytes.length - 1; counter >= 0; counter--) {
      ...
      BigInteger bi_charpos = BigInteger.valueOf("123456789ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz".indexOf(ccc));
    ...
    for (;;)
    {
      byte myByte = vBytes[nPad];
      if (myByte != oneByte) break;
      nPad++;
    }
  ...
  public static String base_encode_58(String v)
  {
    ...
    while (long_value.compareTo(bi_58) > -1) {
      ...
      div = divMod[0];
      mod = divMod[1];
      mychar = "123456789ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz".substring(mod.intValue(), mod.intValue() + 1);
      result = result + mychar;
      long_value = div;
    }
    
    mychar = "123456789ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz".substring(long_value.intValue(), long_value.intValue() + 1);
    result = result + mychar;
  ...
  public static String[] deserialize_xkey(String xkey, boolean prv) {
    ...
    if ((!prv) && 
      (!xkey.substring(0, 8).equals("0488B21E")) && (!xkey.substring(0, 8).equals("0488b21e"))) {
    ...
    if ((prv) && 
      (!xkey.substring(0, 8).equals("0488ADE4")) && (!xkey.substring(0, 8).equals("0488ade4"))) {
      System.out.println("BAD KEY HEADER FAILURE.");
      System.exit(0);
    }
    ...
    String[] retval = { "standard", depth, fingerprint, child_number, c, K_or_k };
  ...
  {
    X9ECParameters ecParameterSpec = ECNamedCurveTable.getByName("secp256k1");
    ...
    String I_hex32 = I_hex.substring(0, 64);
    String I_hex32b = I_hex.substring(64, I_hex.length());
