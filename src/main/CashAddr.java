...
import com.sun.j2me.global.FormatterImpl;
...
public class CashAddr {
	...
	public static String toCashAddress(byte addressType, byte[] hash) {
		...
		byte[] payloadBytes = concatenateByteArrays(new byte[] { addressType }, hash);
	...
	public static BitcoinCashAddressDecodedParts decodeCashAddress(String bitcoinCashAddress) {
		...
		String[] addressParts = StringUtils.split(bitcoinCashAddress, SEPARATOR);
	...
	private static byte getAddressTypeFromVersionByte(byte versionByte) {
		return BitcoinCashAddressType.getVersionByte(versionByte);
	...
	public static boolean isValidCashAddress(String bitcoinCashAddress ) {
		try {
			String prefix;
			if (StringUtils.contains(bitcoinCashAddress , SEPARATOR)) {
				String[] split = StringUtils.split(bitcoinCashAddress, SEPARATOR);
				prefix = split[0];
